        ...
        {user?.links && user.links.length > 0 ? (
          <ul className="flex flex-col items-center lg:items-start gap-3 text-sm font-medium">
            {user.links.map((link, idx) => {
              ...
              if (!href) return null
              const displayUrl = String(href).replace(/^https?:\/\//, '')
              return (
                <li key={idx} className="flex items-center gap-3 w-full justify-center lg:justify-start">
                  ...
                  <a href={href.startsWith('http') ? href : `https://${href}`} target="_blank" rel="noreferrer" className="text-accent hover:text-accent-light transition-colors truncate max-w-[220px] font-bold">
                    {displayUrl}
                  </a>
                </li>
              )
